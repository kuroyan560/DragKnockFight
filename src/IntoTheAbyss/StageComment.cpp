...
void StageComment::Update()
{
	...
	commentSprite->transform.SetPos({ WinApp::Instance()->GetExpandWinSize().x,90.0f });
...
void StageComment::Draw()
{
	...
	Vec2<float>leftUpPos(0.0f, 0.0f);
	Vec2<float>rightDownPos(commentSprite->mesh.GetSize() * Vec2<float>(1.0f, 1.3f));
