	...
	if (isHitTop || isHitRight || isHitLeft || isHitBottom) {
		...
		if (partner.lock()->addSwingAngle <= ADD_SWING_ANGLE * 2.0f) {
			...
			if (partner.lock()->GetNowSwing()) {
				...
				// あたったチップが色ブロックだったら。
				MapChipType hitChipData = StageMgr::Instance()->GetMapChipType(0, 0, hitChipIndex);
				if (hitChipData == MapChipType::MAPCHIP_BLOCK_COLOR_RIGHT) {
...
void CharacterInterFace::OverWriteMapChipValueAround(const Vec2<int>& MapChipIndex, const MapChipType& DstType, const MapChipData& SrcData)
{
	...
	if (StageMgr::Instance()->GetMapChipType(0, 0, MapChipIndex + Vec2<int>(-1, 0)) == DstType) {
		// トゲブロックを棘無し状態にさせる。
		StageMgr::Instance()->WriteMapChipData(0, 0, MapChipIndex + Vec2<int>(-1, 0), SrcData);
	}
	...
	// 指定されたインデックスの左側のチップも左側か右側のブロックかを調べる。
	if (StageMgr::Instance()->GetMapChipType(0, 0, MapChipIndex + Vec2<int>(0, -1)) == DstType) {
		// トゲブロックを棘無し状態にさせる。
		StageMgr::Instance()->WriteMapChipData(0, 0, MapChipIndex + Vec2<int>(0, -1), SrcData);
	}
